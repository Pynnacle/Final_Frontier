...
public class CommandInterface : MonoBehaviour
{
    ...
    public void UpdateDirectionsCrewMemberCanTravel(bool north, bool east, bool south, bool west)
    {
        ...
        //Update the text that displays how many more times the player can move.
        movesRemainingDisplay.text = selectedCrewMember.GetComponent<CrewMember>().moveLimit.ToString();
    ...
    public void GoNorth()
    {
        //Prevent the crew member from moving if they have zero moves remaining.
        if(selectedCrewMember.GetComponent<CrewMember>().moveLimit > 0)
        {
    ...
    {
        selectedCrewMember.GetComponent<CrewMember>().turnIsOver = true; //Prevents the player from being selected for the remainder of the round.
